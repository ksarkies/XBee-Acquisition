...
This bootloader supports microcontrollers without a separate bootloader section.
It should be written as close to the top of FLASH memory as possible. It
requires almost 2K of space, and so is suited only to microcontrollers with 4K
of memory or more. The application code must contain a test of the bootloader
pin and a jump to the bootloader at the start if the bootloader is to be used.
If the bootloader pin is not active the application will continue.

The bootloader must be compiled along with the application. This will ease
access to the bootloader and will allow sharing of common functions.
...
__attribute__ ((OS_main)) int bootloader(void)
{
    uint8_t ch;
