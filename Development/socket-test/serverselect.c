...
// lose the pesky "address already in use" error message (reuse the address)
...
// If we got the socket bound, then can drop out
        break;
    }
...
// listen - we are listening on the interface that was setup
...
// Select changes read_fds to reflect connections pending
// Run through the existing connections looking for data to read
        for(i = 0; i <= fdmax; i++)
		{
            if (FD_ISSET(i, &read_fds)) // we got one!!
			{
printf("Found something waiting %d \n", i);
// Check the local listening socket if a new connection has arrived.
                if (i == listener)
				{
